...
class AgentRnd:
    ...
    def deliberate(self):
        ...
        if not (self.currentState == self.expectedState):
            ...
            if self.podeContinuar(self.currentState.row,self.currentState.col,self.tl):
                ...
                x = self.currentState.row
                y = self.currentState.col
                print("ERRO? agente deve estar em x ", x ,"e y", y)
                currentNodeId = self.plan.getCurrentNodeId()
        ...
        if self.podeContinuar(self.currentState.row,self.currentState.col,self.tl):
            ...
            x = self.currentState.row
            y = self.currentState.col
            print("agente deve estar em x ", x ,"e y", y)
            result = self.plan.returnToBase(self.prob.initialState.col, self.prob.initialState.row)
    ...
    def podeContinuar(self, row, col, tl): # Agente pode continuar explorando ?
        if(row+col+3<tl):
            return True# sim pode vai fundo
        return False# não não pode volta miseria
